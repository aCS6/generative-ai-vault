...
class Configuration:
    def __init__(self):
        ...
        self.redis_url = os.getenv("REDIS_URL", "redis://localhost:6780")
...
class ChatHistory:
    def __init__(self, session_id, redis_url):
        ...
        self.redis_chat_history = RedisChatMessageHistory(
            key_prefix="stickler_ai:",session_id=self.session_id, url=self.redis_url
        )
...
class ProductScriptGenerator:
    ...
    def generate_product_script(
        ...
        if not regenate:
            ...
            Please generate the script below. Ensure that the output contains only the script itself, without any additional commentary or explanations.
